...
│   └── ETL_v0.11_完整处理逻辑.md
├── designs/                            (待实现设计方案)
│   └── DESIGN-001-daily-cost-stock-state-machine.md
├── references/                         (参考手册)
│   ├── strategy_fm_字段手册_完整版.md   (源表字段手册 — 唯一权威完整版)
│   └── strategy_fm_字段手册_BOM版.md   (BOM 专用字段手册)
├── reviews/                            (审查与对比报告)
│   ├── REVIEW-001 ~ REVIEW-007         (6/18-22 QDM 对比 + 矩阵下钻)
...
| 文件 | 内容 |
|------|------|
| [ETL_v0.11_完整处理逻辑.md](architecture/ETL_v0.11_完整处理逻辑.md) | 价格体系 + 14步详解 + 分类重映射 + 字段流转 |

### designs/ — 待实现设计方案

| 文件 | 内容 |
|------|------|
| [DESIGN-001-daily-cost-stock-state-machine.md](designs/DESIGN-001-daily-cost-stock-state-machine.md) | 当日成本-库存状态机改造方案：BOM/加工按父品/原料当日单价定价，期末库存单价=end_stock_amt/end_stock_qty |

### references/ — 参考手册

| 文件 | 说明 |
|------|------|
| [AI_阿里云ETL只读取数指南.md](references/AI_阿里云ETL只读取数指南.md) | 给 AI agent 的生产 DuckDB 只读取数指南，含权限边界、连接方式、查询模板 |
...
| 文件 | 说明 |
|------|------|
| [reviews/README.md](reviews/README.md) | 审查报告索引（REVIEW 索引 + 审查 SOP + **负责人汇报**） |
| [REVIEW-001 ~ REVIEW-008](reviews/) | 6/18-22 QDM 对比下钻系列（最新 REVIEW-008 = 全链路逻辑审计） |
| [ETL项目状态汇报_v0.11_2026-06-24.md](reviews/ETL项目状态汇报_v0.11_2026-06-24.md) | **负责人汇报**：项目状态 + QDM 对比 + 差异原因 + 改进路径 |
