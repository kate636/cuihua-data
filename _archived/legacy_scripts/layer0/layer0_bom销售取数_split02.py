...
def sql_format(start_day,end_day,yesterday):
    ...
    set catalog hive;
    -- 门店商品销售汇总数据
    insert overwrite hive.tmp_dal.dal_store_article_multi_level_bom_splitting_di_02 partition(inc_day)
    select 
     t1.business_date    -- '营业日期'
    ,t1.store_id         -- '门店编码'
    ,t1.abi_article_id as article_id  -- '商品编码'
    ,t2.article_name 
    ,t2.category_level1_id  
    ,t2.category_level1_description  
    ,t1.qty              -- '商品数量信息 标品为份数散称为1规格商品为实际份数'
    ,t1.qty_spec         -- '商品数量信息(规格*份数)'
    ,t1.sales_amt        -- '日结金额'
    ,t1.inc_day
    from
    (
    ...
    ,store_id         -- '门店编码'
    ,abi_article_id   -- '商品编码'
    ,sum(qty) as qty              -- '商品数量信息 标品为份数散称为1规格商品为实际份数'
    ,sum(qty_spec) as qty_spec         -- '商品数量信息(规格*份数)'
    ,sum(sales_amt) as sales_amt        -- '日结金额'
    ,inc_day
    from
    (select 
     business_date    -- '营业日期'
    ,store_id         -- '门店编码'
    ,abi_article_id   -- '商品编码'
    ,qty              -- '商品数量信息 标品为份数散称为1规格商品为实际份数'
    ,qty_spec         -- '商品数量信息(规格*份数)'
    ,sales_amt        -- '日结金额'
    ,inc_day
    from dsl.dsl_transaction_sotre_order_online_details_di 
    ...
    ,sales_amt        -- '日结金额'
    ,inc_day
    from dsl.dsl_transaction_sotre_order_offline_details_di 
    where inc_day between '{start_day}' and '{end_day}'
    )m1 
    group by 
     business_date    -- '营业日期'
    ...
    left join (select * from dim.dim_goods_information_have_pt where inc_day='{yesterday}')t2 
    on t1.abi_article_id=t2.article_id
    ;
    """
