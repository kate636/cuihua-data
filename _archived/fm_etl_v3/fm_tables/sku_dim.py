    ...
    def _build_sql(self, start: str, end: str) -> str:
        ...
        WITH base AS (
            SELECT
                ...
                pft.store_profit_sales,
                pft.store_profit_stock,
                pft.store_profit_diff,
                pft.supply_chain_profit_qty,
                pft.pre_profit_amt,
            ...
            LEFT JOIN (SELECT DISTINCT article_id, category_level1_id, category_level1_description, category_level2_id, category_level2_description, category_level3_id, category_level3_description, sale_unit, unit_weight, spu_id, spu_name, blackwhite_pig_name, article_name FROM dim_goods) g ON w.article_id = g.article_id
            LEFT JOIN (SELECT DISTINCT store_id, manage_area_name, sap_area_name, city_description, store_name FROM dim_store_profile) sp ON w.store_id = sp.store_id
            LEFT JOIN dim_chdj_store_info ch ON w.store_id = ch.store_id
            LEFT JOIN (SELECT DISTINCT business_date, week_no, week_start_date, week_end_date, month_wid, year_wid FROM dim_calendar) cal ON w.business_date = cal.business_date
            LEFT JOIN (SELECT DISTINCT store_id, article_id FROM dim_saleable) sal ON w.store_id = sal.store_id AND w.article_id = sal.article_id
