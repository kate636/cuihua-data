...
def _processing_relations(path: Path | None):
    if path is None:
        # v0.18 must not apply an undated current recipe to historical days.
...
def main() -> int:
    ...
    shadow = subparsers.add_parser(
        "v018-shadow-week", aliases=["v014-shadow-week"],
    ...
    shadow.add_argument("--db", type=Path, default=Path("data/fm_v018_shadow_20260806_20260811.duckdb"))
    shadow.add_argument(
        "--source-db", type=Path, default=Path("data/fm_v018_source_20260805_20260811.duckdb"),
        help="local cache of field-manual Hive mirror projections",
    )
    shadow.add_argument(
        "--stage-db", type=Path, default=Path("data/fm_v018_stage_20260805_20260811.duckdb"),
    ...
    shadow.add_argument(
        ...
        help=(
            "keep exit code 0 after writing a publish-blocked local diagnostic DB; "
            "without this flag a blocked run exits 3"
        ),
    )
    mirror = subparsers.add_parser(
        "v018-fetch-mirrors", aliases=["v014-fetch-mirrors"],
    ...
    stage = subparsers.add_parser(
        "v018-build-stage", aliases=["v014-build-stage"],
    ...
    compare = subparsers.add_parser(
        "v018-compare-v15", aliases=["v014-compare-v15"],
        help=f"compare a local v{__version__} shadow window with the read-only current v1.5 result",
    )
    compare.add_argument("--db", type=Path, default=Path("data/fm_v018_shadow_20260806_20260811.duckdb"))
    compare.add_argument(
        "--comparison-db", type=Path, default=Path("data/fm_v018_comparison_20260806_20260811.duckdb"),
    )
    compare.add_argument(
        "--diagnostic-only", action="store_true",
        help=(
            "keep exit code 0 after writing a failed comparison DB; "
            "without this flag a failed comparison exits 4"
        ),
    )
    args = parser.parse_args()
    if args.command == "preflight":
        return _preflight(args.sync_script)
    if args.command in {"v018-shadow-week", "v014-shadow-week"}:
        ...
        result = run_v014_shadow_week(
            ...
            output_db=args.db,
            mirror_source_db=args.source_db,
        ...
        return 0 if publishable or args.diagnostic_only else 3
    if args.command in {"v018-fetch-mirrors", "v014-fetch-mirrors"}:
        source_end = (
            discover_latest_mirror_day(
                QdmApi(), store_id=args.store, end="auto"
            )
        ...
        return 0
    if args.command in {"v018-build-stage", "v014-build-stage"}:
        mirrors = load_mirror_source_bundle(args.source_db, store_id=args.store)
        ...
        return 0
    if args.command in {"v018-compare-v15", "v014-compare-v15"}:
        ...
        return 0 if not has_failure or args.diagnostic_only else 4
