...
Pipeline (v0.11, 14 步):
  ...
  Step 4   BOM 分摊 (t_calc_bom_alloc)          ← v0.11 修复共享组 + 负值保护
  Step 5   SKU 有效单位成本 (t_calc_sku_cost)    ← v0.11 Python 重写
  Step 6   库存与金额 (t_calc_stock)             ← v0.11 四流合一，Python 重写
  Step 7   门店毛利 (t_calc_profit)              ← v0.11 Python 重写，新公式
...
def _run_calc(duck):
    _step("Step 4: BOM 分摊 → t_calc_bom_alloc [v0.11 修复]")
    BomAllocCalculator(duck).run()

    _step("Step 5: SKU 有效单位成本 → t_calc_sku_cost [v0.11 Python]")
    SkuCostCalculator(duck).run()

    _step("Step 6: 库存与金额 → t_calc_stock [v0.11 四流合一 Python]")
    StockCalculator(duck).run()

    _step("Step 7: 门店毛利 → t_calc_profit [v0.11 Python 新公式]")
