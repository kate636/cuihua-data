...
class DailyFlow:
    init_qty: float = 0.0
    init_amt: float = 0.0
    receive_qty: float = 0.0
    receive_amt: float = 0.0
...
def transition_day(flow: DailyFlow) -> DailyState:
    ...
    nonnegative = {
        "init_qty", "init_amt", "receive_qty", "receive_amt",
    ...
    inflow_qty = (
        flow.init_qty + flow.receive_qty + flow.bom_in_qty + flow.pack_in_qty
        + flow.compose_in_qty + flow.residual_transfer_in_qty
        + flow.sale_return_qty + flow.inventory_gain_qty
    )
    inflow_amt = (
        flow.init_amt + flow.receive_amt + flow.bom_in_amt + flow.pack_in_amt
    ...
    internal_out_qty = (
        ...
        + flow.residual_transfer_out_qty
    )
    internal_out_amt = (
        flow.bom_out_amt + flow.pack_out_amt + flow.compose_out_amt
        + flow.residual_transfer_out_amt
    )
    if internal_out_qty > inflow_qty + 0.001:
        raise ValueError("formal internal out quantity exceeds the available pool")
    if internal_out_amt > inflow_amt + 0.01:
        raise ValueError("formal internal out amount exceeds the available pool")
    ...
    if flow.is_counted:
        ...
        new_supply = (
            flow.receive_qty + flow.bom_in_qty - flow.bom_out_qty
