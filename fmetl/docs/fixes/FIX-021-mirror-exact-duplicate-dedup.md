# FIX-021 镜像提取层完全重复行去重

> 状态：✅已实现 ｜ Commit：`417182f` ｜ ETL验证：✅07-24～30 影子周通过
>
...
仅 `store_name` 不同；而 v0.14 镜像合同的投影列不含 `store_name`，
...
- 适用于全部镜像合同（去重发生在通用提取路径），不改任何合同定义。
...
- [x] 全量单测 141 项通过
- [x] 07-24～30 镜像拉取通过 compose 及后续源表
- [x] 影子周 8 项硬门禁通过
