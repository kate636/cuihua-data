...
### 18. `strategy_dim_store_article_bom_relation` → `atomic_bom_relation`（**v3.1 新增 · P0**）

- **粒度**：门店 × inc_day × parent_article × sub_article（一行是一条 BOM 关系）。
- **2026-04-20 实测**：A3XV 一家 FM 门店 3062 行，`bom_type ∈ {2, 3}`，`sp_level=170`（整表常量）。
- **作用**：解决散称生鲜（典型场景：猪肉 parent = 整头猪 / 白条，sub = 排骨 / 五花 / 梅头 / 碎油）
  的 `cost_price` 缺失问题。下游 `t_calc_bom_cost` 用 parent 的 `avg_purchase_price ×
  compose_out_qty` 作为总成本，按 `dressing_rate × cost_rate` 分摊到 sub。
- **cost_rate 缺失兜底**：BOM 源表 `cost_rate` 为 NULL 或 0 时，用 sub 的 `atomic_price.original_price × dressing_rate`
  在同 parent 下归一化倒推。
- **单位换算**：parent/sub 的 `unit` 可能是 kg 或件，用 `dim_goods.unit_weight` 做换算，最终
  `bom_unit_cost` 统一口径为 **元/kg**，下游 `sale_qty`（qty_spec，kg 口径）直接相乘得销售成本。
- **过滤条件**：`bom_type IN (1, 2, 3)`（不再用 `sp_level` 过滤，该字段在当前数据里是常量 170）。
- **表名注意**：注意实际商分库表名是 `strategy_dim_store_article_bom_relation`（无 `fm_` 前缀，与
  `docs/sql/strategy_fm_bom_relation.sql` 里示例的建表名不一致）。

| `atomic_bom_relation` 字段 | 源字段 | 备注 |
|---|---|---|
| `store_id` | `store_id` | |
| `business_date` | `inc_day` | 按日分区 |
| `parent_article_id` | `parent_article_id` | |
| `sub_article_id` | `sub_article_id` | |
| `parent_unit` | `parent_article_unit` | `kg` / `件` / `公斤` |
| `sub_unit` | `sub_article_unit` | 同上 |
| `dressing_rate` | `dressing_rate` | 出肉率 %（同 parent 下合计 ≈ 100） |
| `cost_rate` | `cost_rate` | 成本比例 %（可能 NULL，走倒推兜底） |
| `bom_type` | `bom_type` | 1 门店 BOM / 2 仓 BOM / 3 非门店非仓 |
| `split_mode` | `split_mode` | 10 一拆多 / 20 一拆一 |
| `sp_level` | `sp_level` | BI 店铺等级（1 实体店） |

**依赖**：先在商分库跑 [sql/strategy_fm_bom_relation.sql](sql/strategy_fm_bom_relation.sql) 把 `strategy_fm_dim_bom_relation` 落库。
...
## 19. `strategy_fm_receive_sale_di` → `atomic_receive_sale`（**v4 新增 · BOM 分摊事实主源**）

### 19.1 粒度与作用

- **粒度**：`(store_id, inc_day, article_id[根 parent], sale_article_id[sub sku])`
- 本表是 QDM 侧**官方 BOM 分摊事实表**，每行是一天里某个根 parent 给某个 sub sku 分到了多少 kg / 多少元。
  v4 以它为 `t_calc_bom_alloc` 的**第一优先级数据源**，压缩了 v3.1.x 自造递归 BOM 的复杂度。

### 19.2 字段对齐

| `atomic_receive_sale` 字段 | 源字段 | 含义 |
|---|---|---|
| `store_id` / `inc_day` | 同名 | 分区键 |
| `business_date` | `inc_day` | 业务日（与 `inc_day` 同义） |
| `article_id` / `article_name` | 同名 | **根 parent**（如"优鲜黑猪 A 级"） |
| `sale_article_id` / `sale_article_name` | 同名 | **sub 销售 sku**（如"优鲜黑猪梅头肉"） |
| `inbound_qty` / `inbound_amount` | 同名 | 父当天入库量 / 金额 |
| `purchase_price` | 同名 | 父入库均价（= `inbound_amount / inbound_qty`） |
| `sum_article_qty` | 同名 | 父入库量去重汇总 |
| `sum_sub_article_qty` | 同名 | 父下所有 sub 分摊到的总 qty |
| `sum_sale_article_qty` | 同名 | 该 sub 当天从所有 parent 拆出的合计 qty |
| `sale_article_qty` | 同名 | 该 sub 从本 parent 拆出的 qty（**核心**） |
| `sale_article_price` | 同名 | 该 sub 进货分摊单价 |
| `split_sale_article_amt` | `spilit_sale_article_amt`（源表拼写瑕疵） | 该 sub 从本 parent 拆出的金额（**核心**） |
| `rate` | 同名 | 该 parent 给 sub 的 qty 占比 |
| `sale_recev_rate` | 同名 | sub_qty / parent_inbound_qty |
| `category_level1_id` / `category_level1_description` | 同名 | 一级品类 |

### 19.3 与 v3 `purchase_di` 的差异

| 维度 | `strategy_fm_purchase_di` | `strategy_fm_receive_sale_di`（**v4 首选**） |
|---|---|---|
| BOM 拆分行 占比 | ~4.5% | **100%** |
| 父 × 子双维度 | 仅 `sale_article_id` 一列 | `article_id`（父）+ `sale_article_id`（子）都有 |
| 分摊金额字段 | `sale_article_purchase_amt` | `spilit_sale_article_amt` + `inbound_amount` |
| 归一化比例字段 | 无 | `rate` / `sale_recev_rate` |

### 19.4 降级策略

- receive_sale_di 当天缺失 → 降级到 `atomic_compose.compose_out_qty` × parent 均价；
- compose 也缺 → `atomic_bom_relation × parent.receive_qty × dressing_rate` 理论值。

---

## 20. `strategy_fm_order_receive_di` → `atomic_order_receive`（**v4 骨架预留，不抽数**）

- **粒度**：门店 × 日期 × sku，每行是订货与验收数量/金额比对。当前上游空。
- v4 已创建 `atomic/order_receive_extractor.py`，但 executor 调用 `ensure_empty_skeleton`
  建空表骨架后就跳过，不实际抽数。
- 预留字段：`order_qty / receive_qty / order_amt / receive_amt / receive_rate`。
- 未来启用时机：供应链履约口径（订货 → 到货 → 拒收）。

---

## 21. `strategy_fm_dim_article_convert` → `atomic_article_convert`（**v4 骨架预留，不抽数**）

- **粒度**：article × 单位换算系数（kg / 件 / 规格）。当前上游空。
- v4 已创建 `atomic/article_convert_extractor.py`，同样走 `ensure_empty_skeleton` 路径。
- 预留字段：`article_id / from_unit / to_unit / convert_ratio / spec_num`。
- 未来启用时机：规格变体 / 套装 / 以件代斤的 sku 统一口径。
