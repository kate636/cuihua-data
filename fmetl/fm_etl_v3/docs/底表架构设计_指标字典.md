...
| **v3.1** | **2026-04-23** | **新增 `atomic_bom_relation` + `t_calc_bom_cost`，解决散称生鲜 sub SKU 成本缺失导致毛利率 = 100% 的口径断点** |

---

## 十、v3.1 新增字段字典

### 10.1 `atomic_bom_relation` · Layer -2 · 原子层（新增）

源表：`strategy_fm_dim_bom_relation`，粒度：**门店 × inc_day × parent_article × sub_article**。

| 字段 | 类型 | 说明 |
|------|------|------|
| `store_id` | varchar | 门店 ID |
| `business_date` | varchar | inc_day，BOM 关系按日分区（品类调整时会改） |
| `parent_article_id` | varchar | 父商品（整头猪 / 白条猪 / 带鱼头鲜等原料） |
| `sub_article_id` | varchar | 子商品（散称排骨 / 五花 / 梅头 / 碎油等成品） |
| `parent_unit` / `sub_unit` | varchar | 计量单位（`kg` / `公斤` / `件` / `袋`） |
| `dressing_rate` | double | 出肉率 %，同 parent 下合计 ≈ 100 |
| `cost_rate` | double | 成本比例 %，可能 NULL（下游用销售原价倒推） |
| `bom_type` | int | 1 门店 BOM / 2 仓 BOM / 3 非门店非仓 |
| `split_mode` | varchar | 10 一拆多 / 20 一拆一 |
| `sp_level` | int | BI 店铺等级（1 实体店） |

### 10.2 `t_calc_bom_cost` · Layer -1 · 计算层（新增）

计算器：`calculated/bom_cost.py · BomCostCalculator`，粒度：**门店 × 业务日期 × sub_article_id**。

| 字段 | 类型 | 说明 |
|------|------|------|
| `store_id`, `business_date`, `article_id` | varchar | article_id = sub_article_id |
| `bom_in_qty_kg` | double | ② 当天 BOM 理论入量（kg 口径） |
| `bom_in_amt` | double | ② 当天 BOM 理论入额（元） |
| `bom_unit_cost` | double | ② **sub 的 BOM 分摊单位成本（元/kg）**，下游 `amounts.py` fallback 用 |
| `parent_count` | int | sub 来自几个 parent（多对一场景 > 1） |
| `rate_source` | varchar | `BOM_RATE` / `INFERRED_BY_PRICE` / `NO_WEIGHT`，DQ 溯源用 |

**公式**：

```
parent_cost_per_kg = avg_purchase_price / (parent_unit_weight if parent_unit='件' else 1)
parent_qty_kg      = COALESCE(compose_out_qty, receive_qty) × (parent_unit_weight if 件 else 1)

# cost_rate 为 NULL 时倒推
inferred_cost_rate[sub] = 100 × (sub.price_per_kg × dressing_rate)
                        / Σ_subs(sub.price_per_kg × dressing_rate)

alloc_qty_kg  = parent_qty_kg × dressing_rate / 100                            -- sub 入量
alloc_amt     = parent_qty_kg × parent_cost_per_kg
              × COALESCE(cost_rate, inferred_cost_rate, 0) / 100               -- sub 入额

-- 多 parent → 同 sub 聚合
bom_in_qty_kg = Σ alloc_qty_kg
bom_in_amt    = Σ alloc_amt
bom_unit_cost = bom_in_amt / bom_in_qty_kg
```

### 10.3 `t_calc_amounts` · 新增字段

| 字段 | 类型 | 说明 |
|------|------|------|
| `effective_unit_cost` | double | ② `COALESCE(NULLIF(cost_price, 0), bom_unit_cost, 0)` |
| `cost_source` | varchar | `DIRECT` (cost_price>0) / `BOM` (走 bom_unit_cost) / `MISSING` (都为 0) |

**影响字段**：`know_lost_amt` / `unknow_lost_amt` / `lost_amt` / `compose_in_amt` / `compose_out_amt` 的 cost 乘数从 `cost_price` 改为 `effective_unit_cost`。

### 10.4 `t_calc_profit` · 新增 / 调整

| 字段 | 调整 |
|------|------|
| `profit_amt` | 日清且无库存/进货/加工流水的 sub（现场切割场景），走 `sale_amt - sale_qty × effective_unit_cost` 兜底 |
| `sale_cost_amt` | 同上兜底；非日清分支 `avg_purchase_price = 0` 时走 `effective_unit_cost` |
| `pre_profit_amt` | 同步兜底 |
| `full_link_article_profit` | 基于 v3.1 `profit_amt` 自动受益 |
| `cost_source` | 直接透传，便于下游 DQ 告警 |

---

## 十一、v4.0 重构（2026-04-23）— Calculated 层拆四张表 + AI 溯源

### 11.1 设计原则

v4 把 v3.1.x 耦合在 `t_calc_amounts` / `t_calc_profit` 里的"BOM 分摊 + 成本解析 + 库存 + 毛利"
四件事拆成四张职责单一的 calculated 表，并在 fm_tables 层补两张专供 AI 溯源的宽表。

```
t_calc_bom_alloc   → BOM 分摊事实（parent × sub）
     ↓
t_calc_sku_cost    → SKU 级 effective_unit_cost + cost_source 四级标签
     ↓
t_calc_stock       → 库存 + 未知损耗三支分流
     ↓
t_calc_profit      → 销售方程 / 库存方程 双落盘 + 供应链 + 全链路
```

### 11.2 `t_calc_bom_alloc` · BOM 分摊事实表（v4 新增）

**粒度**：`(store_id, business_date, parent_article_id, sub_article_id)`

| 字段 | 类型 | 说明 |
|---|---|---|
| `parent_inbound_qty` / `parent_inbound_amount` | double | ① 父当天入库量 / 金额（从 receive_sale_di 或 scm_di 取） |
| `parent_unit_price` | double | ② = `parent_inbound_amount / parent_inbound_qty` |
| `dressing_rate` | double | ① 从 `atomic_bom_relation` 拉出肉率（%） |
| `sub_qty_actual` | double | ① 实际分摊到 sub 的 qty |
| `sub_qty_source` | varchar | `RECEIVE_SALE` / `COMPOSE` / `BOM_THEORETICAL` |
| `sub_alloc_amt` | double | ② sub 当天分摊到的成本金额 |
| `cost_rate_effective` | double | ② 使用的 cost_rate（实际 or 用 `sub.original_price × dressing_rate` 归一化反推） |
| `cost_rate_source` | varchar | `RECEIVE_SALE` / `BOM_DIRECT` / `PRICE_WEIGHT_INFERRED` |
| `sub_unit_cost` | double | ② = `sub_alloc_amt / sub_qty_actual` |

### 11.3 `t_calc_sku_cost` · SKU 有效单位成本（v4 新增）

**粒度**：`(store_id, business_date, article_id)`

| 字段 | 类型 | 说明 |
|---|---|---|
| `effective_unit_cost` | double | ② 四级 COALESCE：DIRECT / BOM_ALLOC / PURCHASE_AVG / MISSING(=0) |
| `cost_source` | varchar | 四种来源 tag |
| `cost_price` / `bom_unit_cost` / `avg_inbound_price` | double | 原始三级取值，便于对账 |
| `parent_count` | int | 该 sub 今天有多少个 parent 给它拆（多 parent 走加权） |

**多 parent 加权**：

```sql
effective_unit_cost = SUM(parent_inbound_qty × dressing_rate × parent_unit_price)
                    / SUM(parent_inbound_qty × dressing_rate)
```

### 11.4 `t_calc_stock` · 库存 + 未知损耗（v4 新增）

**粒度**：`(store_id, business_date, article_id)`

三支分流（⚠️ `day_clear='0'`=日清、`'1'`=非日清、`'2'`=合计）：

```python
if day_clear == '0':        # 日清
    end_stock_qty   = 0
    unknow_lost_qty = init + receive + cin - cout - sale - know_lost
elif know_lost_amt > 0:     # 视为有盘点动作
    end_stock_qty   = init + receive + cin - cout - sale - know_lost
    unknow_lost_qty = 0
else:                       # 非日清('1') 且无盘点：B1-a 纯公式推，假设 unknow=0
    end_stock_qty   = init + receive + cin - cout - sale - know_lost
    unknow_lost_qty = 0
```
...
| 字段 | 说明 |
|---|---|
| `end_stock_amt_self` | 自算 end × effective_unit_cost |
| `end_stock_amt_official` | 取自 `atomic_inventory.end_stock_amt`（purchase_di） |
| `end_stock_amt_diff` | `self - official` 差异 |
| `init_stock_amt_official` | ① 日初必取官方 purchase_di（D1-b，不做接力） |

### 11.5 `t_calc_profit` · 三层毛利（v4 重写）

**粒度**：`(store_id, business_date, article_id)`

| 字段 | 类型 | 说明 |
|---|---|---|
| `store_profit_sales` | double | ② `sale_amt − sale_qty × effective_unit_cost − know_lost_amt − unknow_lost_amt` |
| `store_profit_stock` | double | ② `sale_amt − (receive_amt + compose_in_amt − compose_out_amt) + (end_stock_amt − init_stock_amt) − know_lost_amt − unknow_lost_amt` |
| `store_profit_diff` | double | ② 两口径差（用于定位"哪张单据漏了"） |
| `profit_amt` | double | = `store_profit_sales`（对外对齐基准字段） |
| `sale_cost_amt` | double | ② 库存方程销售成本 |
| `supply_chain_unit_diff` | double | ② `avg_inbound_price − outstock_cost_price`（sub 无 scm → NULL） |
| `supply_chain_profit_qty` | double | ② `sale_qty × supply_chain_unit_diff` |
| `scm_fin_article_profit` | double | ② 供应链财务口径（保留 v3 算法） |
| `full_link_article_profit` | double | ② `store_profit_sales + scm_fin_article_profit` |

### 11.6 fm_tables 层新增两张 AI 溯源表

#### `t_fm_bom_breakdown` — sub × parent 分摊明细

每行 = "某天某 sub sku 从某个 parent 拆到了多少"，全字段暴露：

- `parent_article_id/name`、`parent_inbound_qty/amount`、`parent_unit_price`
- `dressing_rate`、`cost_rate_effective`、`cost_rate_source`
- `sub_qty_actual`、`sub_qty_source`（RECEIVE_SALE / COMPOSE / BOM_THEORETICAL）
- `sub_alloc_amt`、`sub_unit_cost`
- 品类维度（level1/2/3、SPU、黑白猪）

AI 问"SKU-X 今天毛利为负的原因" → 查 `t_fm_bom_breakdown` 可以看到
它当天从几个 parent 拆出来、每个 parent 均价、分摊额，以及走的是哪条兜底路径。

#### `t_fm_stock_roll` — SKU 库存八要素滚动展开

每行 = "某天某 sku 的 8 个库存分量"：

| 分量 | qty 列 | amt 列 |
|---|---|---|
| 期初 | `init_stock_qty` | `init_stock_amt` |
| 进货 | `receive_qty` | `receive_amt` |
| 加工入 | `compose_in_qty` | `compose_in_amt` |
| 加工出 | `compose_out_qty` | `compose_out_amt` |
| 销售 | `sale_qty` | `sale_amt_cost`（= `sale_qty × effective_unit_cost`） |
| 已知损 | `know_lost_qty` | `know_lost_amt` |
| 未知损 | `unknow_lost_qty` | `unknow_lost_amt` |
| 期末自算 | `end_stock_qty` | `end_stock_amt_self` |
| 期末官方 | - | `end_stock_amt_official` |

AI 问"为什么今天期末是 X" → 查 `t_fm_stock_roll` 可以看到八个分量，
以及 `end_stock_amt_diff` 定位哪条链路漏数。

### 11.7 执行顺序（executor v4）

```
atomic/          [12 张] dims + sales + purchase + scm + scm_adjust + loss
                         + compose + allowance + promo + cost_price + price
                         + bom_relation + receive_sale
                         （+ order_receive / article_convert 空骨架）
calc/            [9 步]  merge → inventory → avg_price →
                         bom_alloc → sku_cost → stock →
                         amounts → profit
fm_tables/       [6 张]  sku_dim → cust → levels_sum → levels_result →
                         bom_breakdown → stock_roll
```

### 11.8 v3 vs v4 calculated 层对照

| v3.1.x | v4 | 变化 |
|---|---|---|
| `t_calc_bom_cost`（sub 级） | `t_calc_bom_alloc`（parent × sub 级） | 粒度更细，新增 parent 上下文 |
| `t_calc_amounts.effective_unit_cost` | `t_calc_sku_cost.effective_unit_cost` | 职责独立，新增 PURCHASE_AVG / MISSING tag |
| `t_calc_inventory`（v3 库存方程） | `t_calc_stock`（v4 三支分流） | 日清 / 盘点 / 普通三分支，对齐 B2-a+b+c |
| `t_calc_profit.profit_amt`（单口径） | `t_calc_profit.store_profit_sales/stock`（双口径） | 销售方程 + 库存方程双落盘，对齐 C2-a+b |
