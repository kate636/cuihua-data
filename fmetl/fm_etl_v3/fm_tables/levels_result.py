...
class LevelsResultBuilder:
    ...
    def _build_sql(self, start: str, end: str) -> str:
        ...
        SELECT
            ...
            -- v4 新增：销售方程 / 库存方程双口径
            AVG(store_profit_sales)             AS 门店毛利额_销售方程,
            AVG(store_profit_stock)             AS 门店毛利额_库存方程,
            AVG(store_profit_diff)              AS 门店毛利口径差异,
            AVG(supply_chain_profit_qty)        AS 供应链毛利额_按销量,
            ...
            {safe_div('SUM(article_profit_amt)', 'SUM(total_sale_amt)')}
                                                AS 门店毛利率,
            {safe_div('SUM(store_profit_sales)', 'SUM(total_sale_amt)')}
                                                AS 门店毛利率_销售方程,
            {safe_div('SUM(store_profit_stock)', 'SUM(total_sale_amt)')}
                                                AS 门店毛利率_库存方程,
