...
class LevelsSumBuilder:
    ...
    def _build_full_sql(self, start: str, end: str) -> str:
        ...
        # 将 levels_sum 基础结果 LEFT JOIN t_fm_cust，补全来客数 / 19 点前客数 / 动销 sku 数
        # cust_wide: exact day_clear + '2'(合计) 两种
        return f"""
        ...
        LEFT JOIN cust_wide c
          ...
          -- 大类 level_id 在 levels_sum 是数字/空串(sku_dim 重映射)，
          -- 但 CustBuilder 用映射后的"大类名称"作为 level_id；
          -- 因此大类维度必须用 description 做 key，其它层保持 level_id 对齐。
