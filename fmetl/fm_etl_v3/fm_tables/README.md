...
Pipeline 的 **Step 11~16**（v4.0）：从 DuckDB 的计算结果表出发，构建面向 FM 平台看板、分析师和 AI 溯源的 6 张最终底表。
...
|---|---|---|---|
| `t_fm_sku_dim` | `SkuDimBuilder` | 门店 × 日期 × article_id × day_clear | SKU 级完整宽表（含 v4 新列 `store_profit_sales/stock/diff` + `cost_source` + `effective_unit_cost`） |
| `t_fm_cust` | `CustBuilder` | 门店 × 日期 × day_clear × level | 按 6 个层级聚合的**客数** |
| `t_fm_levels_sum` | `LevelsSumBuilder` | 门店 × 日期 × 分类等级 × day_clear | 7 层分类 UNION ALL 展开 |
| `t_fm_levels_result` | `LevelsResultBuilder` | 同 levels_sum | **最终对接看板**（中文列名 + 比率型 KPI，v4 新增 3 列毛利） |
| `t_fm_bom_breakdown` **v4 新** | `BomBreakdownBuilder` | 门店 × 日期 × parent × sub | **AI 溯源**：sub sku × parent 的 BOM 分摊明细（parent 均价 / 出肉率 / 分摊额 / 来源 tag） |
| `t_fm_stock_roll` **v4 新** | `StockRollBuilder` | 门店 × 日期 × sku × day_clear | **AI 溯源**：库存八要素（init/receive/compose_in/out/sale/know_lost/unknow_lost/end）滚动展开 |
...
| 门店毛利额_销售方程 | — | `store_profit_sales`（v4 新增） |
| 门店毛利额_库存方程 | — | `store_profit_stock`（v4 新增） |
| 门店毛利口径差异 | — | `store_profit_diff`（v4 新增：销售方程 − 库存方程） |
| 供应链毛利额_按销量 | — | `supply_chain_profit_qty`（v4 新增） |
...
## Step 15 · `bom_breakdown.py` — `BomBreakdownBuilder`（v4 新增）

**DuckDB 表**：`t_fm_bom_breakdown`  
**粒度**：门店 × 日期 × parent_article_id × sub_article_id

从 `t_calc_bom_alloc` 关联 `dim_goods`（parent 和 sub 各一次）+ 分类重映射得到。
...
-- 为什么今天 "优鲜黑猪梅头肉" 毛利偏低？
SELECT
  parent_article_name, parent_inbound_qty, parent_unit_price,
  dressing_rate, cost_rate_effective, cost_rate_source,
  sub_qty_actual, sub_qty_source, sub_alloc_amt, sub_unit_cost
FROM t_fm_bom_breakdown
WHERE sub_article_id = '20110932'
  AND business_date = '2026-04-20';
```

## Step 16 · `stock_roll.py` — `StockRollBuilder`（v4 新增）

**DuckDB 表**：`t_fm_stock_roll`  
**粒度**：门店 × 日期 × article_id × day_clear

从 `t_calc_stock` + `t_calc_amounts` + `t_atomic_wide` 合并得到库存八要素完整滚动。
...
SELECT
  ...
  sale_qty, know_lost_qty, unknow_lost_qty,
  end_stock_qty, end_stock_amt_self, end_stock_amt_official, end_stock_amt_diff
FROM t_fm_stock_roll
WHERE article_id = '20110932'
  AND business_date = '2026-04-20';
```
