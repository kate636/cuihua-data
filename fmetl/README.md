...
- 包装关系本身不能造流。v0.17 只使用专用的当日换码明细，已删除通过盘点反推包装的活跃链路。
...
完整执行分支和本次 2026-08-06～11 的 461 条原因明细见
...
- 大分类同时保留“各自实际分类”和“v0.17 冻结快照归一”视图；分类快照证据闭合前只作诊断。
...
| 分类快照证据 | 只覆盖至 2026-07-20，阻止发布 |
| 服务器写入 | 0 |

本次是基于本地 Hive 镜像、当前经营平台加工关系和只读 v1.5 参照的完整新鲜重算。
结果通过全部会计硬门禁，但没有通过两项发布证据门禁，不能把诊断输出包装成正式生产结果。
...
- 决定系统库存快照是否具有盘点权威性；当前继续只审计，不覆盖期末。
- 生成覆盖 2026-08-06～11 的不可变分类快照，不能沿用只覆盖至 07-20 的证据冒充同期事实。
