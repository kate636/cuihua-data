...
fmetl v0.17 是花家市集商品关系、库存、成本和门店毛利的本地影子 ETL。
源码直接位于现有 `fmetl/`，没有单独建立 v014 代码目录。

v0.17 从 Hive 对应的 StarRocks 镜像读取业务事实，自行完成商品关系判定、内部转移、
加权成本、库存滚动、损耗和毛利计算。v1.5 只提供输出字段、维度和聚合合同，并用于
只读差异验证；v1.5 的库存、成本、损耗和毛利结果不进入 v0.17 计算。
...
- 加工与生熟联动原料报损二选一：同原料同日同时命中时，生熟联动优先，整组加工不过账。
- 同一 A→B 已由规范化外部验收正数量入账时，外部验收优先，正式加工关系只审计、不过账。
- 加工消耗不以原料账面余额为门槛；余额不足进入日账负库存钉零和透支成本，不把已发生消耗改成零。
- 系统库存快照只作审计，不冒充人工盘点。
...
3. **关系与数量事实分离**：静态关系说明“可以怎样转”，`purchase_di`、销售或报损说明“当天实际多少”；不要求虚构一张转换明细表。
...
- `source` 和 `stage` 保存 D-1；正式结果、SKU 日账、关系登记、正式内部过账
  和异常表只保存发布区间。新窗口事务替换旧兼容物理表，因此不会混入上一次窗口。
...
| 分类 | 经营监控平台最新有效映射 | 全运行窗口统一使用最新日期分类；未命中 SKU 才回退最新 `dim_goods` |
...
- 正式 BOM 与正式加工没有静默优先级；同日同关系对命中多个内部类型时直接隔离。
...
- 包装/换码不依赖不存在的明细表。固定 A→B 关系负责比例，`purchase_di` 负责当天 B 数量和采购金额；B 直接作为外部验收入账。已删除通过盘点反推包装的活跃链路。
...
- 关系在业务日有效；
- 缺生效日期、未批准或不在有效期的关系只隔离该行，不扩大为全表停算；
- 同一成品当日只有一组有效配方；
- 所有原料与成品关系都在统一关系登记表中正式生效；
- 同日原料没有生熟联动报损；若有，生熟联动优先，加工整组不过账。
- 同一 A→B 没有已经实际入账的外部验收正数量；若有，外部验收优先，加工整组不过账。
...
当日成本池为空但发生销售、报损或内部转出时，不允许默认 0 成本。v0.17 按以下证据顺序回退：
...
- 负盘点、小数点疑似错位会先被移出正式盘点并进入隔离；同商品组验收码与销售码同时盘点时只忽略验收码盘点，保留销售码盘点。
...
`v014_sku_daily` 是兼容物理表名，保留 v0.17 内部账本真值。生成公共结果时：

- 全部 SKU 日的期初、期末和会计毛利都使用 v0.17 关系账本结果；Hive 普通库存快照只作审计；
- 炒菜机金额从损耗扣除并加回各层兼容展示毛利；
- 生熟联动从损耗扣除，只在大分类执行来源类加回、熟食类扣减，门店、SPU 和 SKU 毛利不增加；
- 展示调整不修改内部库存、单位成本和 `v014_internal_posting`。

关系修正保留在 v0.17 账本，兼容展示调整不反向修改会计底项。

## 11. 分类与聚合

### 11.1 大分类口径与证据

v0.17 当前保留冻结的 v1.5 基线规则：
...
`CATEGORY_SNAPSHOT_EVIDENCE` 阻止发布。对比库同时保留各自实际分类和 v0.17 冻结分类归一两种视图。
...
| BOM 公共数量不守恒 | 整个事件隔离 |
| 加工关系缺比例、比例非正数或同一成品同日命中多个配方版本 | 整组成品加工隔离 |
| 同一原料同日已有生熟联动报损 | 生熟联动优先，整组加工不生成事件 |
| 同一 A→B 已有规范化外部验收正数量 | 外部验收优先，保留关系审计，整组加工不生成事件 |
| 加工原料账面余额不足 | 不阻止加工；日账期末钉零并单列透支成本 |
| 加工自环关系 | 标记 `PROCESSING_SELF_RELATION_NOOP`，不生成无意义内部事件 |
| 同金额验收明细可证明采购数量 | 只补采购数量，保留采购金额；金额不一致仍隔离 |
| 负盘点 | 保留原值审计，期末按库存方程计算 |
| 盘点疑似小数点错位 | 盘点失效并隔离 |
| 验收码与销售码重复盘点 | 忽略验收码盘点并隔离，保留销售码盘点 |
| 库存方程为负 | 期末钉零，透支成本单列 |
...
8. `PROCESSING_RAW_LOSS_PRIORITY`
9. `PROCESSING_EXTERNAL_RECEIPT_PRIORITY`
10. `BOM_PARENT_FULLY_TRANSFERRED`
...
flowchart LR
    ...
    EXACT --> NORMALIZED_CATEGORY["两边 SKU 按 v0.17 冻结快照归一"]
...
- 大分类同时保留“各自实际分类”和“v0.17 最新经营监控平台分类归一”视图；同一最新快照统一映射整个运行窗口。
...
│   └── graph.py                       同日关系拓扑排序
├── facts/
│   ├── formal_events.py               BOM 事件与兼容的显式转换事件合同
│   ├── processing_inference.py        销售/普通报损加工反冲
...
本次是基于本地 Hive 镜像、当前经营平台加工关系和只读 v1.5 参照的完整新鲜重算。
结果通过全部会计硬门禁和分类证据门禁；193 个缺成本 SKU 日仍使成本发布门禁失败，不能把诊断输出包装成正式生产结果。
...
- 为 193 个缺成本 SKU 日补充合法成本事实，或由业务明确批准带期限的历史成本回退策略。
- 确认 4 个“一成品多关系 ID”究竟是多原料共同配方还是互斥替代配方；确认前继续隔离。
- 处理经营平台 27 条加工自环关系；当前安全地视为无动作，不参与成本。
- 系统库存快照已确认不具有盘点权威性；继续只审计，不覆盖期末。
- 分类已按经营监控平台最新日期快照统一映射；需保留每次运行冻结快照以确保可复现。
- 确认 A3XV 商品集通用区域快照的生产权威性，并补齐仍无正式比例的候选转码。
- 完成生产并行、回滚演练和单独发布审批。
