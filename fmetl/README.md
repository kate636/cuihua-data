...
v0.17 目前只在本地试算。它读取 Hive 在 StarRocks 中的镜像数据，自己计算商品关系、库存、
...
- 加工与生熟联动原料报损二选一：同一配方原料当天命中生熟联动时，保留生熟联动报损，
  该配方当天不再扣加工原料。
...
- StarRocks 提供 Hive 数据的访问镜像；每个字段最初来自哪张 Hive 表，以字段手册为准。
...
| 层级 | 主要内容 | 物理位置 | 约束 |
|---|---|---|---|
| 原始数据缓存 | Hive 镜像记录、商品集和固定换码关系 | `fm_v014_source.duckdb` | 只读抽取，记录数据来源、行数和校验值 |
| 标准化数据 | 把不同来源统一成相同字段、日期、商品和正负号 | `fm_v014_stage.duckdb` | 只由程序生成，不接受人工改表 |
| 商品关系清单 | 可用关系、待确认关系、冲突关系和问题原因 | `v014_relation_registry` | 同一来源码→目标码→业务日最多选一条正式处理路径 |
| 商品转换明细 | 来源商品转出、目标商品转入及当天数量依据 | `v014_internal_posting` | 每次转换两边必须齐全，转出金额必须等于转入金额 |
| 每日库存账 | 每个 SKU 每天的数量、成本、库存、损耗和毛利 | `v014_sku_daily` | 每天都有一行；今天期末原样成为明天期初 |
...
- 镜像按“源表 × 日期”分批读取；成功的日期立即保存，中断后只重读未完成日期。
...
| 数据来源 | 能说明什么 | 不能说明什么 |
|---|---|---|
| 商品集快照 + 商品重量 | 两个商品可能是同一商品的不同编码或规格；可以计算重量换算参考比 | 当天是否转换、当天实际转换多少；重量参考比不能替代正式关系比例 |
| 正式 BOM | 哪个父品可以拆成哪些子品，以及建议比例 | 门店当天实际拆了多少 |
...
- `v014_stage_activities`：商品日销售、退回、验收、报损、盘点和日清。
...
- 正式 BOM 和正式加工没有默认优先级；同一对商品同一天同时命中多种关系时，不选择其中
  一种，这对商品不计入当天转换。
...
| `UNRESOLVED` / `CONFLICT` | 没有关系依据，或同时命中多种关系 | 不计入转换，写入问题清单 |
...
- 全部商品日的期初、期末和会计毛利都使用 v0.17 关系账本结果；Hive 普通库存快照只供检查；
...
- 每层分别生成日清、非日清和合计三种结果，合计由最细的商品日记录重新汇总。
...
| 问题 | 处理 |
|---|---|
| 同一对商品同一天命中多种关系 | 整对标记 `CONFLICT`，不设置默认优先级 |
| 正式关系缺数量比例或成本比例 | 不计算，不补一个默认比例 |
...
该命令检查包版本、镜像字段结构、分类规则，以及同步脚本所需字段和命令；不读取或写入生产结果。
...
| `data/fm_v014_shadow.duckdb` | 125 字段结果、每日库存账、商品关系、商品转换明细、问题和检查结果 |
...
├── cli.py                              当前命令入口
├── mirror/
│   ├── registry.py                    镜像表、字段和原始 Hive 表的对应关系
...
│   ├── daily_cost_stock.py            单商品日库存分支计算
│   └── profit.py                      会计毛利
├── outputs/
...
| 输出行数 | 88,996 |
| 必须通过的账目检查 | 10/10 通过 |
| v1.5 商品日销售额、销售量 | 完全一致 |
| v1.5 门店日验收金额 | 完全一致 |
| 门店周毛利 | v0.17 25,611.95；v1.5 27,346.50；差 -1,734.55（-6.34%） |
| 缺成本 | 193 个商品日、52 个商品；当天没有可用且尚未计入的正数成本，因此阻止发布 |
| 分类来源 | 经营监控平台最新 2026-08-12；1,449 SKU 匹配，11 SKU 使用最新 `dim_goods`，检查通过 |
| 服务器写入 | 0 |

本次使用本地保存的 Hive 镜像、当前经营平台加工关系和只读 v1.5 参照，从期初准备日开始完整重算。
结果通过全部账目检查和分类来源检查；193 个缺成本商品日仍使发布条件失败，本地规则检查
结果不得作为正式生产结果。
